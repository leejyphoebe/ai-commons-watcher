...
add_cron_job "0 1 * * Mon-Fri" "$cmd 'NSCC Usage AM Report'" "$logs_dir/cron_nscc_am_\`$date_cmd +\%Y\%m\%d\`.log"

# Add a cron job to run a command at 6pm on weekdays and log output
add_cron_job "0 9 * * Mon-Fri" "$cmd 'NSCC Usage PM Report'" "$logs_dir/cron_nscc_pm_\`$date_cmd +\%Y\%m\%d\`.log"
